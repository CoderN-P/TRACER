<script lang="ts">
    ...
    import { List, Filter, CheckCircle2, AlertTriangle, Info, XCircle, ArrowDownToLine, Search } from 'lucide-svelte';
    import { scale, fly, fade } from 'svelte/transition';
    import { quintOut } from 'svelte/easing';
    
    let { logs, class: className = "" }: { logs: LogEntry[], class?: string } = $props();
        ...
        <Card.Header>
            ...
            <div class="mt-2 flex flex-col sm:flex-row gap-2 sm:items-center">
                ...
                <div class="relative flex-grow">
                    ...
                    />
                    {#if searchTerm}
                        <button class="absolute inset-y-0 right-0 pr-2 flex items-center text-gray-400 hover:text-gray-600"
                                on:click={() => searchTerm = ""}>
                ...
                <div class="flex items-center gap-1 flex-shrink-0">
                    <button class="flex items-center gap-1 px-2 py-1 text-xs rounded-md transition-colors
                                   {activeFilter === null ? 'bg-gray-200 text-gray-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}"
                    ...
                    <button class="flex items-center gap-1 px-2 py-1 text-xs rounded-md transition-colors
                                   {activeFilter === 'warning' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}"
                            on:click={() => toggleFilter('warning')}>
                        <AlertTriangle class="h-3 w-3" />
                        <span>{logCounts.warning}</span>
                    </button>
                    
                    <button class="flex items-center gap-1 px-2 py-1 text-xs rounded-md transition-colors
                                   {activeFilter === 'check' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}"
                            on:click={() => toggleFilter('check')}>
                        <CheckCircle2 class="h-3 w-3" />
                        <span>{logCounts.check}</span>
                    </button>
                    
                    <button class="flex items-center gap-1 px-2 py-1 text-xs rounded-md transition-colors
                                   {activeFilter === 'info' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}"
                            on:click={() => toggleFilter('info')}>
                        <Info class="h-3 w-3" />
                        <span>{logCounts.info}</span>
                    </button>
                    
                    <button class="flex items-center gap-1 px-2 py-1 text-xs rounded-md transition-colors
                                   {activeFilter === 'error' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}"
                            on:click={() => toggleFilter('error')}>
        ...
        <div class="relative">
            ...
            >
                {#if filteredLogs.length === 0}
                    <div class="flex flex-col items-center justify-center py-8 text-gray-400" in:fade>
                ...
                {#if logs.length > 10 && !searchTerm && !activeFilter}
                    <div class="sticky top-0 w-full flex justify-center py-1 z-10" in:fade={{ duration: 200 }}>
                        <div class="flex items-center gap-1 px-2 py-1 text-xs bg-gray-100 text-gray-500 rounded-full">
