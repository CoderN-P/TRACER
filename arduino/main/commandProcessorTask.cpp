    ...
    while (true)
    {
        ...
        if (xQueueReceive(commandQueue, buffer, portMAX_DELAY) == pdPASS)
        {
            ...
            do
            {
                ...
                if (typeIndex != -1)
                {
                    uint8_t cmdLen = expectedCommandLength(type);
                    if (cmdLen >= 2 && cmdLen <= MAX_BUFFER_SIZE)
                    {
                        memcpy(latestCmds[typeIndex], buffer, cmdLen);
            ...
            for (int i = 0; i < NUM_TYPES; i++)
            {
                if (typeReceived[i])
                {
                    uint8_t cmdLen = expectedCommandLength(latestCmds[i][1]);
                    if (cmdLen >= 2 && cmdLen <= MAX_BUFFER_SIZE)
                    {
                        handleCommand(latestCmds[i], cmdLen);
                    }
                }
            }
        }
