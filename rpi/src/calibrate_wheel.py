...
def calibrate_wheel(speed, duration_sec):
    ...
    with lock:
        ...
        logging.info(f"Measure the true distance traveled by the wheel (e.g. by marking the wheel and counting rotations), then calculate the correction factor as: correction_factor = true_distance / average_distance for each motor to get corrected versions of METERS_PER_TICK_LEFT and METERS_PER_TICK_RIGHT.")
