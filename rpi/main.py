...
from src import Robot, SerialManager, run_socket_server, socketio, ROBOT_CONFIG, calibrate_wheel, calibrate_wheelbase, serial_test, calibrate_ks, calibrate_kv, calibrate_max_speed, interactive_velocity_test, calibrate_mag, interactive_pid_test
...
parser.add_argument('--interactive-velocity-test', action='store_true', help="Run interactive velocity test CLI (enter left/right m/s + duration repeatedly until exit).")
parser.add_argument("--timeout", type=float, default=1.0, help="No-data timeout before mag calibration (seconds)")
parser.add_argument('--interactive-pid-test', action='store_true', help="Run interactive PID testing CLI")
async def main(p):
...
if __name__ == "__main__":
    ...
    elif args.interactive_velocity_test:
        interactive_velocity_test(args.port)
    elif args.interactive_pid_test:
        interactive_pid_test(args.port)
