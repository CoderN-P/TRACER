...
class Command(BaseModel):
    ...
    pause_duration: int = Field(default=0, description="Pause duration in seconds after executing the command")
    duration: int = Field(default=0, description="Duration in seconds for which the command should be executed")
    ...
    def send_from_joystick(cls, left_y: float, right_x: float, ser: SerialManager):
        ...
        """
        left_motor  = int(-left_y * 255 * (1 - right_x))
        right_motor = int(-left_y * 255 * (1 + right_x))
